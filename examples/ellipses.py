...
# Create a custom contour filter for fitting ellipses to the largest contour
def custom(contours):
    c = hcv.largest(contours)
    arr = np.zeros(diffImg.shape, dtype='uint8')
    x,y,w,h = bbox
    arr = arr[y:y+h, x:x+w]
    arr = cv2.ellipse(arr, (cv2.fitEllipse(c)), 255, -1)
    c,_ = hcv.util.find_contours(arr)
    return c

# Initiate an instance of heartcv.Segmentation with the embryo outline and heart bounding box 
# found above.
segment = hcv.Segmentation(video, contour, bbox)
...
# segmentation method
layer = segment.gui(inv=True, contour_method=custom)
print(layer)

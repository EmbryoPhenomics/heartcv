...
@app.callback(Output("filepath-check", "children"), [Input("filename-input", "value")])
...
@app.callback(
    Output("hcv-filepath-check", "children"), [Input("hcv-filename-input", "value")]
)
...
@app.callback(
    ...
    [Input("cardiac-cycle", "value"), Input("frame-table", "data")],
)
...
@app.callback(
    ...
    [Input("filename-input", "value"), Input("frame-number", "value")],
)
...
@app.callback(
    [Output("selected-data", "children"), Output("compute-callback", "children")],
    [
...
def update_image(update, compute, fr):
    if fr:
        ...
        if frame is not None:
            return html.Div(
                children=[app_utils.interactiveImage(id="still-image-graph", img=frame)]
            )
        else:
